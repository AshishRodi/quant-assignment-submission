...
with st.sidebar:
    ...
    if data_source == "Live Stream (Simulation)":
        ...
        # KEY CHANGE: Checkbox is more stable than a button for infinite loops
        is_running = st.checkbox("✅ Enable Live Stream", value=True)
...
if data_source == "Live Stream (Simulation)":
    # 1. Update Data if Running
    if is_running:
        current_df = st.session_state['live_data']
        last_row = current_df.iloc[-1]
        new_time = last_row['timestamp'] + pd.Timedelta(minutes=1)
        
        # Random Walk Logic (Volatile to ensure signals)
        move_a = np.random.randn() * 60 
        move_b = (move_a * 0.05) + (np.random.randn() * 20) 
        ...
        updated_df = pd.concat([current_df, new_row], ignore_index=True).tail(300)
...
if not df.empty:
    ...
    if hedge_ratio:
        ...
        with tab1:
            ...
            # Using unique key based on timestamp to force redraw
            st.plotly_chart(fig, use_container_width=True, key=f"main_chart_{len(df)}")
        ...
        with tab3:
            ...
            st.download_button("⬇️ Download CSV", csv, "analytics.csv", "text/csv", key=f"dl_{len(df)}")

# --- 5. AUTO-REFRESH LOGIC ---
if data_source == "Live Stream (Simulation)" and is_running:
    time.sleep(1) # Wait 1 second
    st.rerun()    # Force the entire app to reload with new data